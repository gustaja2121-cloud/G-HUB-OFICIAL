...
export default function App() {
  ...
  // Force logout on initial load so the app always starts on the login screen
  useEffect(() => {
    if (user) {
      logout();
    }
  ...
  // Test connection on mount (kept from original code)
  useEffect(() => {
  ...
  return (
    <ToastProvider>
      <div className="min-h-screen bg-bg text-white flex flex-col md:flex-row pb-24 md:pb-0 font-sans selection:bg-accent/30">
        {/* Sidebar for Desktop */}
        <aside className="hidden md:flex fixed left-0 top-0 bottom-0 w-80 bg-surface/50 backdrop-blur-2xl border-r border-white/5 flex-col z-50 overflow-y-auto custom-scrollbar">
          {/* Logo Section */}
        ...
        <main className="flex-1 w-full p-6 md:p-12 lg:p-16 overflow-y-auto max-h-screen custom-scrollbar selection:bg-accent/20">
          <AnimatePresence mode="wait">
            {activeTab === 'finance' && (
              <motion.div key="finance" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-6xl mx-auto">
                <Finance onNavigate={setActiveTab} />
              </motion.div>
            )}
            {activeTab === 'ranking' && (
              <motion.div key="ranking" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-6xl mx-auto">
            ...
            {activeTab === 'notes' && (
              <motion.div key="notes" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="max-w-6xl mx-auto">
                <Notes onNavigate={setActiveTab} />
              </motion.div>
            )}
          </AnimatePresence>
        </main>
